...
class Checkpoints(Logger):
    ...
    def __init__(self, path: str, error_every=1, config_every=1, model_every=1):
        ...
        for f in glob.glob(self.path.__str__() + '/*', recursive=True):
            os.remove(f)
        self.error_logger = self.Errors(self, log_every=error_every)
        self.config_logger = self.Configs(self, log_every=config_every)
        self.model_logger = self.Models(self, log_every=model_every)
        self.loggers = [self.error_logger, self.config_logger, self.model_logger]
